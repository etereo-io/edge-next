...
import { DATABASES, FIELDS, INTERACTION_TYPES } from '@lib/constants'
...
const InteractionSchema = object({
  type: string()
    ...
    .oneOf([INTERACTION_TYPES.LIKE, INTERACTION_TYPES.FOLLOW], 'Invalid interaction type'),
...
const FieldSchema = object({
  ...
  type: string()
    ...
    .matches(
      `(${FIELDS.TAGS}|${FIELDS.TEXT}|${FIELDS.TEXTAREA}|${FIELDS.IMAGE}|${FIELDS.FILE}|${FIELDS.SELECT}|${FIELDS.NUMBER}|${FIELDS.JSON}|${FIELDS.BOOLEAN}|${FIELDS.RADIO}|${FIELDS.VIDEO_URL}|${FIELDS.MARKDOWN}|${FIELDS.DATE}|${FIELDS.URL}|${FIELDS.TEL}|${FIELDS.ENTITY_SEARCH})`,
      'Invalid field type'
    ),
...
export default object({
  ...
  superSearch: object({
    enabled: boolean(),
    permissions: object(),
    entities: array(object({name: string(), fields: array(string()), permissions: array(string()), type: string(), fieldsForShow: array(string())}))
  }),
})
