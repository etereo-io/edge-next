import { DATABASES, ROLES, ROLES_LIST } from '@lib/constants'
...
export default function load() {
  try {
    ...
    if (newConfig.superSearch && newConfig.superSearch.permissions) {
      ...
      // interactions
    ;(newConfig.user.entityInteractions || []).forEach(({ type: interactionType, permissions }) => {
    ...
    newConfig.content.types.forEach((type) => {
      ...
      // interactions
      (type.entityInteractions || []).forEach(({ type: interactionType, permissions }) => {
        Object.entries(permissions).forEach(([key, value]) => {
          availablePermissions[
