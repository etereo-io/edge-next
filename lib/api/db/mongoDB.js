...
class db extends Database {
  ...
  find({id, ...options}) {
    ...
    return new Promise((resolve, reject) => {
      this.db.collection(this.col).find(options).skip(this.startIndex).limit(this.limitIndex).toArray((err, result) => {
        if(err) {
          reject(err)
        } else {
  ...
  doc(id) {
    ...
    return {
      set: (newItemData) => {
        return new Promise((resolve, reject) => {
          this.db.collection(this.col).findOneAndUpdate({_id: id}, newItemData, (err, item) => {
            if (err) {
              reject(err)
            } else {
              if (item) {
