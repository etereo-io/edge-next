...
export async function createUser({ username, email, password, tokens = [], instagram = null, profile = {} }) {
...
export async function findUser({ email, password }) {
  ...
  if (!user) {
    ...
    if (user && !user.password) {
      return Promise.reject('You have registered using a sign-in provider. To enable password login, sign in using a provider, and then set a password under your user profile.')
    }
